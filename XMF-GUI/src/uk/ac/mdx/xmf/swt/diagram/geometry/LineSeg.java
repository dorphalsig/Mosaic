...
public class LineSeg implements Cloneable, java.io.Serializable, Translatable {
	...
	public final boolean containsPoint(final Point aPoint, final int tolerance) {
		...
		return (theOrigin.getDistance(aPoint) + aPoint.getDistance(theTerminus) <= length()
				+ tolerance);
	}
	...
	public PointList getLinesIntersections(LineSeg line) {
		...
		if (det == 0) {
			...
			intersections.addPoint(new Point(Math.round((c1 * b2 - b1 * c2)
					/ det), Math.round((a1 * c2 - c1 * a2) / det)));
		}
	...
	public PointList getLineIntersectionsWithEllipse(Rectangle ellipseBounds) {
		...
		if (b == 0) {
			...
			if (y < 0)
				return intersections;
			intersections.addPoint(new Point(Math.round(x
					+ ellipseBounds.getCenter().x), Math.round(Math.sqrt(y)
			...
			intersections.addPoint(new Point(Math.round(x1
					+ ellipseBounds.getCenter().x), Math.round((c - a * x1) / b
					+ ellipseBounds.getCenter().y)));
			intersections.addPoint(new Point(Math.round(x2
					+ ellipseBounds.getCenter().x), Math.round((c - a * x2) / b
					+ ellipseBounds.getCenter().y)));
		}
	...
	public PointList getLineIntersectionsWithLineSegs(final PointList points) {
		...
		for (int i = 0; i < points.size() - 1; i++) {
			LineSeg line = new LineSeg(points.getPoint(i).getCopy(), points
					.getPoint(i + 1).getCopy());
			...
			for (int j = 0; j < currentIntersections.size(); j++) {
				...
				if (line.containsPoint(intersection,
	...
	public Point movePointOnSeq(Point p, int units) {
		...
		Point ref;
		if (origin.getDistance(p) > terminus.getDistance(p))
			ref = origin;
		else
			ref = terminus;
		double length = length();
		double normX = (terminus.x - origin.x) / length;
		double normY = (terminus.y - origin.y) / length;

		double newX;
		double newY;

		if (ref.x < p.x)
			newX = p.x - (normX * units);
		else
			newX = p.x + (normX * units);
		if (ref.y < p.y)
			newY = p.y - (normY * units);
		else
			newY = p.y + (normY * units);
