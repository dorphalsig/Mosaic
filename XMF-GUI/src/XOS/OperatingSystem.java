...
public final class OperatingSystem implements EventHandler {
	...
	private final Vector clientThreads = new Vector();
	...
	private final Vector inputChannels = new Vector();
	...
	private final Vector inputMonitors = new Vector();
	...
	private final Vector outputChannels = new Vector();
	...
	private final ThreadQueue scheduledThreads = new ThreadQueue();

	// A pool of free values used in messages.

	private final Stack valuePool = new Stack();
	...
	private final Vector waitingMessageClients = new Vector();

	// XOS contains a single XVM that is used to run each of the
	// threads.

	private final Machine XVM = new Machine(this);
	...
	private final String[] XOSargSpecs = { "-port:1", "-debug:0",
			"-internal:1", "-message:1" };
	...
	public Value call(String name, Message message) {
		...
		if (client != null) {
			if (message != null) {
				...
				// writeText(message.name(),
				// "F:\\xmf\\code\\messageCallOut.txt");
			}
	...
	// String[] test = { "-arg", "user:XModeler", "-arg",
	...
	public void init(String[] args) {
		...
		// String[] s = null;
		// // String s = gson3.toJson(arg);
		// //
		// // writeText(s, "F:\\xmf\\code\\args.txt");
		// try {
		// s = readTxt("F:\\xmf\\code\\args.txt");
		// } catch (IOException e) {
		// // TODO Auto-generated catch block
		// e.printStackTrace();
		// }
		//
		// arg = gson3.fromJson(s[0], Args.class);
		//
		// args = arg.value;
		parseArgs(args);
	...
	public synchronized void newExternalClient(Socket socket) {
		...
		try {
			...
			if (client(name) != null) {
				...
				// // ---yong
				// String[] messages = name.split("-");
				//
				// for (int i = 0; i < messages.length; i++) {
				//
				// String messageName = messages[i];
				//
				// Message obj = gson2.fromJson(messageName, Message.class);
				// raiseEvent("com.ceteva.diagram", obj);
				//
				// // M2MSend send = new M2MSend(socket);
				// // send.start();
				//
				// writeText(messageName, "f:\\xmf\\code\\test.txt");
				//
				// }

				// String fClient="F:\\xmf\\code\\clientTest.txt";
				// String message="F:\\xmf\\code\\messageTest.txt";
				//
				// String[] clientNames=null;
				// String[] messages=null;
				//
				// try {
				// clientNames=readTxt(fClient);
				// } catch (IOException e) {
				// // TODO Auto-generated catch block
				// e.printStackTrace();
				// }
				// try {
				// messages=readTxt(message);
				// } catch (IOException e) {
				// // TODO Auto-generated catch block
				// e.printStackTrace();
				// }
				...
				// raiseEvent(name,obj);
				//
				// writeText(name,"f:\\xmf\\code\\log.txt");
				// writeText(messageName,"f:\\xmf\\code\\log.txt");
				// }
				// ---end
			}
		} catch (IOException ioe) {
	...
	// br.close();
	// }
	// Message obj = gson2.fromJson(everything, Message.class);
	//
	// raiseEvent("com.ceteva.diagram",obj);
	// writeText(everything,"f:\\xmf\\code\\log.txt");
	// //---end
	// }
	// } catch (IOException ioe) {
	// System.out.println(ioe);
	...
	public void raiseEvent(String name, Message message) {
		...
		// //---yong
		//
		// if (name.equalsIgnoreCase("com.ceteva.diagram")&&!flag){
		// flag=true;
		// String fClient="F:\\xmf\\code\\clientTest.txt";
		// String fmessage="F:\\xmf\\code\\messageTest.txt";
		//
		// String[] clientNames=null;
		// String[] messages=null;
		//
	...
	public Message readMessage(String name) {
		...
		if (client != null) {
			...
			if (message != null) {
				// String messageString = "";
				// messageString=gson.toJson(message);
				// writeText(messageString,"F:\\xmf\\code\\messageOut.txt");
			}
			return message;
		} else
			return null;
	...
	public synchronized void waitForMonitor() {
		if (scheduledThreads.isEmpty()) {
			...
			try {
				...
				System.out.println("waitformonitor:" + e);
			}
		}
	}
