...
public class JavaTranslator {
	...
	public static int mapJavaArray(Machine machine, int[] values) {
		int seq = Machine.nilValue;
		for (int i = values.length - 1; i >= 0; i--)
			seq = machine.mkCons(mapJavaValue(machine, new Integer(values[i])),
					seq);
		return seq;
	}
	...
	public static int mapJavaArray(Machine machine, float[] values) {
		int seq = Machine.nilValue;
		for (int i = values.length - 1; i >= 0; i--)
			seq = machine.mkCons(mapJavaValue(machine, new Float(values[i])),
					seq);
		return seq;
	}
	...
	public static int mapJavaArray(Machine machine, double[] values) {
		int seq = Machine.nilValue;
		for (int i = values.length - 1; i >= 0; i--)
			seq = machine.mkCons(mapJavaValue(machine, new Double(values[i])),
					seq);
		return seq;
	}
	...
	public static int mapJavaArray(Machine machine, boolean[] values) {
		int seq = Machine.nilValue;
		for (int i = values.length - 1; i >= 0; i--)
			seq = machine.mkCons(mapJavaValue(machine, new Boolean(values[i])),
					seq);
		return seq;
	}
	...
	public static int mapJavaHashtable(Machine machine, Hashtable hashTable) {
		...
		while (keys.hasMoreElements()) {
			...
			machine.hashTablePut(table, mapJavaValue(machine, key),
					mapJavaValue(machine, value));
		}
		return table;
	}
	...
	public static int mapJavaVector(Machine machine, Vector vector) {
		...
		for (int i = vector.size() - 1; i >= 0; i--)
			seq = machine.mkCons(mapJavaValue(machine, vector.elementAt(i)),
					seq);
		return seq;
	}
	...
	public static int mapJavaValue(Machine machine, Object value) {
		...
		if (value instanceof Boolean)
			return ((Boolean) value).booleanValue() ? Machine.trueValue
					: Machine.falseValue;
	...
	public static Object mapXMFConsToArray(Machine machine, Class type,
			int value) {
	...
	public static int[] mapXMFConsToIntArray(Machine machine, int values) {
		...
		while (values != Machine.nilValue) {
			...
			if (Machine.isInt(value)) {
				...
				throw new XMFToJavaTypeError("Expecting an int", value,
						Integer.TYPE);
		}
		return array;
	}
	...
	public static boolean[] mapXMFConsToBoolArray(Machine machine, int values) {
		...
		while (values != Machine.nilValue) {
			...
			if (Machine.isBool(value)) {
				...
				throw new XMFToJavaTypeError("Expecting a boolean", value,
						Boolean.TYPE);
		}
		return array;
	}

	public static Object[] mapXMFConsToObjectArray(Machine machine, Class type,
			int values) {
	...
	public static Vector mapXMFConsToVector(Machine machine, int cons) {
		...
		while (cons != Machine.nilValue) {
			vector.addElement(mapXMFValue(machine, Object.class,
					machine.consHead(cons)));
	...
	public static Object mapXMFValue(Machine machine, Class type, int value) {
		if (value == Machine.undefinedValue)
			if (type.isPrimitive() || type == java.lang.Object.class)
				throw new XMFToJavaTypeError("Primitive type cannot be null",
						value, type);
			else
				return null;
		if (Machine.isInt(value))
			if (type == Integer.TYPE || type == java.lang.Integer.class
		...
		if (Machine.isCons(value))
			if (type.isArray())
				return mapXMFConsToArray(machine, type.getComponentType(),
						value);
		...
		if (Machine.isAlienObj(value)) {
			...
			if (type.isInstance(object))
				...
				throw new XMFToJavaTypeError("Alien obj of wrong type", value,
						type);
		}
		if (Machine.isForeignObj(value)) {
			...
			if (type.isInstance(object))
				...
				throw new XMFToJavaTypeError("Foreign obj of wrong type",
						value, type);
		}
		return null;
	}
