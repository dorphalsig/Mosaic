...
public class DialogsClient extends Client {
  ...
  public static Value newInputDialog(final String title, final String message, final String value) {
    ...
    return result[0] != null ? new Value(result[0]) : new Value(false);
  }
  ...
  public static Value openMultiSelectionDialog(String title, String message, Value[] options) {
    ...
    ListDialog dialog = new ListDialog(shell, title, message, stringOptions);
    String[] chosen = dialog.open(100, 100);
    if (chosen == null)
      return new Value("-1");
    else return new Value(getResultArray(chosen, defaultOptions));

    /*
     * 
     * ListDialog ld = new ListDialog(shell); ld.setInput(allOptions); ld.setContentProvider(new ArrayContentProvider()); ld.setLabelProvider(new LabelProvider()); ld.setMessage(message); ld.setTitle(title); ld.setInitialSelections(defaultOptions); if (ld.open() != SWT.CANCEL) { Object[] result =
     * ld.getResult(); if (result != null && result.length > 0) return new Value(getResultArray(result, defaultOptions)); } return new Value("-1");
     */
  }
  ...
  public static Value openSelectionDialog(String title, String message, Value[] options) {
    ...
    String[] defaultOptions = processDefaultOptions(stringOptions);
    Shell shell = XModeler.getXModeler();
    org.eclipse.ui.dialogs.ListDialog ld = new org.eclipse.ui.dialogs.ListDialog(shell);
  ...
  private Value newQuestionDialogYesOnly(final Message message) {
    runOnDisplay(new Runnable() {
      public void run() {
        Value question = message.args[0];
        new MessageDialog(XModeler.getXModeler(), "Question", null, question.strValue(), MessageDialog.INFORMATION, new String[] { "OK" }, 0).open();
      }
    });
  ...
  private Value newQuestionDialogYesNoCancel(final Message message) {
    ...
    runOnDisplay(new Runnable() {
      public void run() {
        ...
        // Value defaultResponse = message.args[1];
        // Value icon = message.args[2];
        MessageDialog md = new MessageDialog(XModeler.getXModeler(), "Question", null, question.strValue(), MessageDialog.QUESTION_WITH_CANCEL, new String[] { "Yes", "No", "Cancel" }, 0);
        switch (md.open()) {
          case 0:
   ...
   */
  public TreeElement buildTree(Value[] tree, Vector<TreeElement> expand, Vector<TreeElement> disable, Vector<TreeElement> select) {
   ...
   *          the select
   */
  public void buildTree(TreeElement parent, Value[] tree, Vector<TreeElement> expand, Vector<TreeElement> disable, Vector<TreeElement> select) {
    // A
  ...
  /*
   * public Value multiTreeDialog(Message message) { String title = message.args[0].strValue(); Value[] tree = message.args[1].values; Vector expand = new Vector(); Vector disable = new Vector(); Vector select = new Vector(); TreeElement root = buildTree(tree, expand, disable, select);
   * MultiSelectionTreeDialog treeDialog = new MultiSelectionTreeDialog(XModeler.getXModeler(), new LabelProvider(), new TreeElementProvider()); treeDialog.setTitle(title); treeDialog.setInput(root); treeDialog.create(); treeDialog.expandTree(expand); treeDialog.disableNodes(disable);
   * treeDialog.selectNodes(select); int returncode = treeDialog.open(); if (returncode != 1) { Object[] result = treeDialog.getResult(); if (result != null) { Value[] values = new Value[result.length]; for (int i = 0; i < result.length; i++) { TreeElement te = (TreeElement) result[i]; Vector path =
   * new Vector(); te.getPath(path); Value[] value = new Value[path.size()]; for (int z = path.size(); z > 0; z--) { String s = (String) path.elementAt(z - 1); value[path.size() - z] = new Value(s); } values[i] = new Value(value); } return new Value(values); } } return new Value(""); }
   */
  ...
  public Value colorDialog(final Message message) {
    ...
    DialogsClient.theClient().runOnDisplay(new Runnable() {
      public void run() {
        ...
        dialog.setText(text);
        if (red > 0 && green > 0 && blue > 0) dialog.setRGB(new RGB(red, green, blue));
