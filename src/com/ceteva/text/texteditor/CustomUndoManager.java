 ...
 * @see KeyListener
 */
public class CustomUndoManager implements IUndoManager {
	...
	class TextCommand {
		...
		protected void undoTextChange() {
			try {
				fTextViewer.getDocument().replace(fStart, fText.length(),
						fPreservedText);
		...
		/**
		 * Undo the change described by this command. Also selects and reveals
		 * the change.
		 */
		protected void undo() {
			undoTextChange();
			selectAndReveal(fStart,
					fPreservedText == null ? 0 : fPreservedText.length());
		}
		...
		/**
		 * Updates the command stack in response to committing the current
		 * change into this command.
		 */
		...
		/**
		 * Creates a new uncommitted text command depending on whether a
		 * compound change is currently being executed.
		 * 
		 * @return a new, uncommitted text command or a compound text command
		 */
		protected TextCommand createCurrent() {
			return fFoldingIntoCompoundChange ? new CompoundTextCommand()
					: new TextCommand();
		}
	...
	/**
	 * Represents an undo-able edit command consisting of several individual
	 * edit commands.
	 */
		...
		if (listen) {
			if (fDocumentListener == null && fTextViewer.getDocument() != null) {
				...
				fTextViewer.getDocument()
						.addDocumentListener(fDocumentListener);
			}
		} else if (!listen) {
			if (fDocumentListener != null && fTextViewer.getDocument() != null) {
				fTextViewer.getDocument().removeDocumentListener(
						fDocumentListener);
	...
	private boolean isWhitespaceText(String text) {
		...
		String[] delimiters = fTextViewer.getDocument()
	...
	/**
	 * Returns the state the would result if the current editing command would
	 * be closed.
	 * 
