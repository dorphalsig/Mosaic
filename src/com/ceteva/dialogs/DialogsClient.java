...
public class DialogsClient extends Client {
	...
	public Value orderingDialog(Message message) {
		...
		// Object[] objs =
		// Arrays.asList(message.args).subList(2,message.arity).toArray();
		Object[] objs = message.args[2].values;
		String[] sources = new String[objs.length];
		for (int i = 0; i < objs.length; i++)
			sources[i] = ((Value) objs[i]).strValue();
		OrderingDialog d = new OrderingDialog(getShell(), text, message_,
				sources);
	...
	public void textAreaDialog(Message message) {
		...
		TextAreaDialog tad = new TextAreaDialog(getShell(), "Message", image,
				message_, text, imageType, new String[] { "OK" }, 0);
		tad.open();
	}
	...
	public Value selectionDialog(Message message) {
		...
		if (multi)
			return SelectionDialog.openMultiSelectionDialog(title, message_,
					options, handler);
		else
			return SelectionDialog.openSelectionDialog(title, message_,
					options, handler);
	}
