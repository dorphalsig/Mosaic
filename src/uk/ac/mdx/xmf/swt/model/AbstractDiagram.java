...
public class AbstractDiagram extends Container implements Droppable {
	 ...
	 */
	public AbstractDiagram(ClientElement parent, EventHandler handler,
			String identity) {
		super(parent, handler, identity, new Point(0, 0), new Dimension(0, 0),
				null, null);
	}
	...
	public boolean processMessage(Message message) {
		if (super.processMessage(message))
			return true;
		if (message.hasName("exportImage") && message.arity == 3
		...
		if (message.hasName("copyToClipboard")
		...
		if (message.hasName("zoomTo") && message.args[0].hasStrValue(identity)
				&& message.arity == 2) {
			int percent = message.args[1].intValue;
			zoomTo(percent);
			return true;
		} else if (message.args[0].hasStrValue(identity)
				&& (message.hasName("enableDrop") && message.arity == 1)) {
			enableDrop();
			return true;
		}
	 ...
	 *
	 * @param queuedZoom the new queued zoom
	 */
