...
public class ModelFactory {
	...
	public static Box newBox(ClientElement parent, EventHandler handler,
		...
		int fillBlue = message.args[16].intValue;// if hegith==0, did not draw
		return new Box(parent, handler, identity, x, y, width, height, curve,
				top, right, bottom, left,
				getColor(lineRed, lineGreen, lineBlue), getColor(fillRed,
						fillGreen, fillBlue));
	}
	...
	public static void newBox(Container parent, Element box) {
		...
		Box b = new Box(parent, parent.handler, identity, x, y, width, height,
				curve, top, right, bottom, left);
		parent.addDisplay(b);
		b.synchronise(box);
	}
	...
	public static void newText(Container parent, Element text) {
		...
		Text t = new Text(parent, parent.handler, id, x, y, label, editable,
				underline, italicise, font);
		parent.addDisplay(t);
		t.synchronise(text);
	}
	...
	public static MultilineText newMultilineText(ClientElement parent,
		...
		return new MultilineText(parent, handler, identity, text, x, y, width,
				height, editable, getColor(foreRed, foreGreen, foreBlue),
	...
	public static void newMultilineText(Container parent, Element multiline) {
		...
		MultilineText t = new MultilineText(parent, parent.handler, id, label,
				x, y, width, height, editable, font);
		parent.addDisplay(t);
	...
	public static MultilineEdgeText newMultilineEdgeText(ClientElement parent,
		...
		return new MultilineEdgeText(parent, handler, identity, position, x, y,
				text, editable, underline, condense,
	...
	public static EdgeText newEdgeText(ClientElement parent,
		...
		int blue = message.args[11].intValue;
		String font = message.args[12].strValue();
		return new EdgeText(parent, handler, identity, position, x, y, text,
	...
	public static Line newLine(ClientElement parent, EventHandler handler,
		...
		return new Line(parent, handler, identity, x1, y1, x2, y2, getColor(
				red, green, blue));
	}
	...
	public static Ellipse newEllipse(ClientElement parent,
		...
		return new Ellipse(parent, handler, identity, x, y, width, height,
	...
	public static void newEllipse(Container parent, Element ellipse) {
		...
		Ellipse e = new Ellipse(parent, parent.handler, id, x, y, width,
				height, outline);
		parent.addDisplay(e);
	...
	public static Image newImage(ClientElement parent, EventHandler handler,
		...
		int width = message.args[5].intValue;
		int height = message.args[6].intValue;
		return new Image(parent, handler, identity, filename, x, y, width,
				height);
	}
	...
	public static Shape newShape(ClientElement parent, EventHandler handler,
		...
		RGB lineColor = getColor(lineRed, lineGreen, lineBlue);
		RGB fillColor = getColor(fillRed, fillGreen, fillBlue);
		Value[] rawPoints = message.args[13].values;
		...
		return new Shape(parent, handler, identity, x, y, width, height,
				outline, points, lineColor, fillColor);
	}
}