...
class HeadFactory {
	...
	private static RotatableDecoration ball() {
		...
		for (int i = 0; i < 360; i++) {
			...
			Point p = new Point(Math.cos(degInRad) * radius, Math.sin(degInRad)
					* radius);
			p.translate(150, 0);
	...
	private static RotatableDecoration claw() {
		 ...
		 * decorationPointList.addPoint(0, 0); decorationPointList.addPoint(0,
		 * -2); decorationPointList.addPoint(1, -2);
		 * decorationPointList.addPoint(0, -2); decorationPointList.addPoint(0,
		 * 2); decorationPointList.addPoint(1, 2);
		 * decoration.setTemplate(decorationPointList); return decoration;
		 */
