 ...
 ****************************************************************************/
package uk.ac.mdx.xmf.swt.diagram.geometry;
...
public class PointListUtilities {
	 ...
	 * 
	 * @param points
	 *            the <code>PointList</code> which will be modified to remove
	 *            redudant points
	 * @param straightLineTolerance
	 *            the tolerance value within which indicates the line is
	 *            straight
	 * @return <code>boolean</code> <code>true</code> if segments were modified,
	...
	static boolean flattenSegments(PointList points, int straightLineTolerance) {
		...
		for (int i = 0; i < points.size() - 1; i++) {
			...
			// We conservatively avoid changing the endpoint of the line, since
			// it may then
			// fail to attach to the terminal view. This implies ptNext must be
			// non-NIL.
	...
	/**
	 * Normalizes the line segments in the polyline. Checks for lines that are
	 * with-in a threshold for length and removes them. Additionally, it will
	 * remove points that that adjacent segments that are both horizontal, or
	 * both vertical.
	 * 
	 * @param points
	 *            <code>PointList</code> to be normalized
	 * @return <code>boolean</code> <code>true</code> if segments were changed,
	 *         <code>false</code> otherwise
	 */
	 ...
	 * with-in a threshold for length and removes them. Additionally, it will
	 * remove points that that adjacent segments that are both horizontal, or
	 * both vertical. Will utilize a tolerance value to determine if segments
	 * needs to flattened or not.
	 * 
	 * @param points
	 *            <code>PointList</code> to normalize
	 * @param straightLineTolerance
	 *            the tolerance value within which indicates the line is
	 *            straight in relative coordinates.
	 * @return <code>boolean</code> <code>true</code> if segments were changed,
	 *         <code>false</code> otherwise
	 */
	static public boolean normalizeSegments(PointList points,
		...
		// second pass to flatten segments that are parallel to each other
		// with-in a tolerance value
	...
	/**
	 * Method getPointsSupremum. Get points representing the extrema for this
	 * poly line.
	 * 
	 * @param points
	...
	static public Point getPointsSupremum(PointList points) {
		...
		for (int i = 1; i < points.size(); i++) {
			...
			theSupremum = new Point(Math.max(theSupremum.x, other.x), Math.max(
					theSupremum.y, other.y));
		}
	...
	/**
	 * Method getPointsSupremum. Get points representing the minimum for this
	 * poly line.
	 * 
	 * @param points
	...
	static public Point getPointsInfimum(PointList points) {
		...
		for (int i = 1; i < points.size(); i++) {
			Point other = points.getPoint(i);
			theInfimum = new Point(Math.min(theInfimum.x, other.x), Math.min(
					theInfimum.y, other.y));
		}
	...
	/**
	 * reAdjustBoxSize private utility method to shrink / expand the box segment
	 * size to accommodate the given point on the edge. When completed the
	 * containedPoint will be on the border of the boxSegs list. Assumptions: 1.
	 * boxSegs is a list of LineSegs that represent a rectangle. 2. boxSegs is
	 * ordered clockwise around the rectangle.
	 * 
	 * @param boxSegs
	 *            List of LineSegs to shrink / expand
	 * @param containedPoint
	 *            Point that is typically contained inside the Rectangle the the
	 *            boxSegs LineSeg list represents.
	 */
	static private void reAdjustBoxSize(List boxSegs, Point containedPoint) {
		// assert boxSegs.size() == 4;

		LineSeg seg = getNearestSegment(boxSegs, containedPoint.x,
				containedPoint.y);
	...
	/**
	 * Method routeAroundRect. Route this polyline around a rectangle that is
	 * presumably intersecting with it.
	 * 
	 * @param points
	 *            PointList that will be modified to route around the given
	 *            rectangle
	 * @param rBox
	 *            the <code>Rectangle</code> around which the routing will
	 *            occur.
	 * @param nSmoothFactor
	 *            the <code>int</code> smooth factor to route the line with 0 -
	 *            None, 15 - some, 30 - lots
	 * @param bIncludeIntersectionPoints
	 *            the <code>boolean</code> determining whether to include the
	 *            points that intersect with the rectangle in the newly routed
	 *            polyline.
	 * @param nBuffer
	 *            the <code>int</code> buffer around the rectangle.
	 * @return <code>PointList</code> that is the newly routed version of
	 *         <code>points</code> of <code>null</code> if operation was not
	 *         successful or if the calculation is not possible.
	 */
	static public PointList routeAroundRect(PointList points,
			final Rectangle rBox, int nSmoothFactor,
			final boolean bIncludeIntersectionPoints, final int nBuffer) {
		...
		if (rPoly.intersects(rBox)) {
			...
			// check to see if the end points of the list are contained in the
			// box.
			...
			return routeAroundPoly(points, rBoxPoly, nSmoothFactor, true,
					bIncludeIntersectionPoints, nBuffer);
		}
	...
	/**
	 * Constant that is the default number of lines that a bezier is
	 * approximated by as a polyline point list.
	 */
	...
	/**
	 * Routes this polyline around another polyline that is presumably
	 * intersecting with it.
	 * 
	 * @param points
	 *            the <code>PointList</code> that is to be modified based on the
	 *            routing calculations made around the <code>poly</code>
	 *            parameter.
	 * @param poly
	 *            the <code>PolylinePointList</code> around which the routing
	 *            will occur.
	 * @param nSmoothFactor
	 *            the <code>int</code> smooth factor to route the line with 0 -
	 *            None, 15 - some, 30 - lots
	 * @param bShortestDistance
	 *            the <code>boolean</code> determining whether to use the
	 *            shortest distance possible to achieve this or else minimally
	 *            modify the existing polyline.
	 * @param bIncludeIntersectionPoints
	 *            the <code>boolean</code> determining whether to include the
	 *            points that intersect with the rectangle in the newly routed
	 *            polyline.
	 ...
	 *         successful or if the calculation is not possible.
	 */
	static public PointList routeAroundPoly(PointList points,
			final PointList poly, int nSmoothFactor, boolean bShortestDistance,
			final boolean bIncludeIntersectionPoints, final int nBuffer) {
		...
		while (listIter.hasNext()) {
			...
			while (boxIter.hasNext()) {
				...
				// check if this segment intersects with the box - intersect
				// with 0 tolerance
				ptIntersect = pSegment.intersect(pBoxSegment,
						INTERSECT_TOLERANCE);
				if (ptIntersect != null) {
					// check case where intersect is on a corner - causing
					// intersect
					...
					if (Math.abs(ptPrevIntersect.x - ptIntersect.x) > (INTERSECT_TOLERANCE * 2)
						...
						if (ptIntersect1 != null) {
							...
							int nCurrentLength2 = nCurrentLength
									+ (int) (pSegment
											.distanceAlong(ptIntersect) * pSegment
											.length());
							...
							getRoutedPoints(points, newRoutePoints,
									ptIntersect1, ptIntersect2,
									rPolyPoints.getLastPoint(),
									pSegment.getTerminus(), pCurBoxSeg,
									pCurBoxSeg2, poly, nSmoothFactor,
									bShortestDistance,
									bIncludeIntersectionPoints, nBuffer);
							...
							for (int i = 0; i < newRoutePoints.size(); i++)
								rPolyPoints
										.addPoint(newRoutePoints.getPoint(i));
							rPolyPoints.addPoint(new Point(pSegment
									.getTerminus()));
							...
							pCurBoxSeg = pBoxSegment;
							rPolyPoints
									.addPoint(new Point(pSegment.getOrigin()));
	...
	/**
	 * copyPoints This method is necessary because of an apparrent defect in the
	...
	static private int addRoutedPoints(PointList routePoints,
			LineSeg pCurBoxSeg1, LineSeg pCurBoxSeg2,
			final PointList routePoly, final boolean bForward, final int nBuffer) {
		...
		ListIterator boxIter = bForward ? rBoxSegments.listIterator()
				: rBoxSegments.listIterator(rBoxSegments.size());
		while (bForward ? boxIter.hasNext() : boxIter.hasPrevious()) {
			LineSeg seg = bForward ? (LineSeg) boxIter.next()
		...
		do {
			...
			if (nBuffer > 0)
				pNewPoint = new Point(pCurBoxSeg1.locatePoint(fOffset, nBuffer,
			...
			if (!pCurBoxSeg1.equals(pCurBoxSeg2)) {
				if (bForward) {
					...
					if (!boxIter.hasPrevious())
						boxIter = rBoxSegments
								.listIterator(rBoxSegments.size());
	...
	static private void getRoutedPoints(PointList points,
			...
			final Point ptIntersect2, final Point ptPrev, final Point ptNext,
			LineSeg pCurBoxSeg1, LineSeg pCurBoxSeg2,
			final PointList routePoly, int nSmoothFactor,
		...
		if (nBuffer > 0) {
			...
			ptAbove = pCurBoxSeg1.locatePoint(dDistance, nBuffer,
					LineSeg.Sign.POSITIVE);
		}
		...
		// if we're not including the intersection points we still need to
		...
		nDistance1 += addRoutedPoints(newRoutePoints1, pCurSeg, pCurBoxSeg2,
				routePoly, true, nBuffer);
		nDistance1 += newRoutePoints1.getLastPoint().getDistance(ptIntersect2);

		pCurSeg = pCurBoxSeg1;
		nDistance2 += addRoutedPoints(newRoutePoints2, pCurSeg, pCurBoxSeg2,
				routePoly, false, nBuffer);
		...
		if (nBuffer > 0) {
			...
			ptAbove = pCurBoxSeg2.locatePoint(dDistance, nBuffer,
					LineSeg.Sign.POSITIVE);
		}
		...
		// check for redundant points and remove them from the list if
		// necessary.
		if (!bIncludeIntersectionPoints && newRoutePoints.size() >= 3) {
			...
			while (ptEnd != null) {
				...
				// check if we should add the pptCheckSkip to the routePoints or
				// not
				Point ptIntersect = new Point();
				boolean bAddPoint = false;

				// if the temporary line seg does intersect with the routePoly,
				// this means
				...
				while (segIter.hasNext()) {
					...
					ptIntersect = tempSeg.intersect(pPolySegment,
							INTERSECT_TOLERANCE);
					if (ptIntersect != null) {
						bAddPoint = true;
						break;
		...
		if (nSmoothFactor > 0) {
			PointList tempPoly = calcSmoothPolyline(newRoutePoints,
					nSmoothFactor, DEFAULT_BEZIERLINES);
			copyFrom(newRoutePoints, tempPoly);
		}
	}

	/**
	 * Method calcSmoothPolyline. Calculate the smooth polyline approximation of
	 * this polyline based on a smooth factor.
	 * 
	 * @param points
	 *            the <code>PointList</code> that is used to calculate the
	 *            smooth point list from.
	 * @param nSmoothFactor
	 *            the <code>int</code> smooth factor to smooth the line with 0 -
	 *            None, 15 - some, 30 - lots
	 * @param nBezierSteps
	 *            the <code>int</code> number of line steps used to approximate
	 *            the smooth curve
	 * @return PolylinePointList List of PolylinePoint representing the smooth
	 *         polyline.
	 */
	static public final PointList calcSmoothPolyline(PointList points,
			int nSmoothFactor, int nBezierSteps) {
		PointList theBezierPoints = calcBezier(points, nSmoothFactor, 0,
				points.size() - 1);
		return calcApproxPolylineFromBezier(theBezierPoints, nBezierSteps);
	}

	/**
	 * Calculates the smooth polyline equivalent of the given points list. It
	 * will extrapolate a bezier approximation in polylines that resembles a
	 * curved line.
	 * 
	 * @param points
	 *            the <code>PointList</code> that is used to calculate the
	 ...
	 *            the smooth curve
	 * @param nStartIndex
	 *            the <code>int</code> index in the line to start from to create
	 *            the smooth approximation
	 * @param nEndIndex
	 *            the <code>int</code> index in the line to end at to create the
	 *            smooth approximation
	 * @return PolylinePointList List of PolylinePoint representing the smooth
	 *         polyline.
	 */
	static public PointList calcSmoothPolyline(PointList points,
			int nSmoothFactor, int nBezierSteps, int nStartIndex, int nEndIndex) {
		PointList theBezierPoints = calcBezier(points, nSmoothFactor,
				nStartIndex, nEndIndex);
		return calcApproxPolylineFromBezier(theBezierPoints, nBezierSteps);
	}

	/**
	 * Calculate the actual bezier points of this polyline based on a smooth
	 * factor.
	 * 
	 * @param points
	 *            PointList to calculate the bezier approximation from
	 * @param nSmoothFactor
	 *            the <code>int</code> smooth factor to smooth the line with 0 -
	 *            None, 15 - some, 30 - lots
	 * @param nStartIndex
	 *            int Index to start the calculation at
	 * @param nEndIndex
	 *            int Index to end the calculation at
	 * @return PolylinePointList List of PolylinePoint representing the bezier.
	 */
	static private PointList calcBezier(final PointList points,
			int nSmoothFactor, int nStartIndex, int nEndIndex) {
		Point ptPrevControl = null;
		PointList theBezierPoints = new PointList(points.size() * 2);
		// parse through the line segments and create control points based on a
		// smoothing factor.
			...
			if (dLineLength > MIN_LINE_LENGTH) {
				...
				if (ptPrevControl != null) {
					...
					prevControlSeg.pointOn(
							(int) Math.round(prevControlSeg.length()
									+ dControlLength), LineSeg.KeyPoint.ORIGIN,
							ptStartControl);
				} else {
					ptStartControl = new Point();
					pLineSeg.pointOn((int) Math.round(dControlLength),
							LineSeg.KeyPoint.ORIGIN, ptStartControl);
				}
				if (bAddToBezier)
				...
				if (pNextSeg != null) {
					...
					Ray ptVector1 = new Ray(pLineSeg.getOrigin(),
							pLineSeg.getTerminus());
					Ray ptVector2 = new Ray(pNextSeg.getOrigin(),
							pNextSeg.getTerminus());
					...
					Point ptVector1Prime = trans.getTransformed(new Point(
							ptVector1.x, ptVector1.y));

					LineSeg nextControlSeg = new LineSeg(new Point(0, 0),
					...
					nextControlSeg.pointOn((int) Math.round(dControlLength),
							LineSeg.KeyPoint.ORIGIN, ptProjection);

					ptTerminusControl = new Point(pLineSeg.getTerminus().x
							- ptProjection.x, pLineSeg.getTerminus().y
							- ptProjection.y);
				} else {
					pLineSeg.pointOn(
							(int) Math.round(dLineLength - dControlLength),
							LineSeg.KeyPoint.ORIGIN, ptTerminusControl);
				}
	...
	/**
	 * Utility function that takes a set of bezier points and calculates a
	 * polyline approximation.
	 * 
	 * @param points
	 *            the <code>PointList</code> to calculate the bezier from.
	 * @param nBezierSteps
	 *            the <code>int</code> number of line steps that will be used to
	 *            approximate each bezier curve
	 * @return PolylinePointList List of PolylinePoint representing the smooth
	 *         polyline.
	 */
	static private PointList calcApproxPolylineFromBezier(
			final PointList points, int nBezierSteps) {
		PointList thePolyPoints = new PointList(points.size() * nBezierSteps
				+ 2);
		...
		for (int i = 0; i < points.size() - 3; i = i + 3) {
			...
			if (!BezierToLines(thePolyPoints, ptCtl1, ptCtl2, ptCtl3, ptCtl4,
					nBezierSteps))
				return null;
		...
		thePolyPoints.setPoint(points.getPoint(points.size() - 1),
	...
	private static boolean BezierToLines(PointList thePolyPoints, Point ptCtl1,
			Point ptCtl2, Point ptCtl3, Point ptCtl4, int nSteps) {
		boolean bRC = true;
		...
		while (nLinePoints <= nSteps) {
			...
			/*
			 * average consecutive coordinates and put average between
			 * coordinates
			 */
		...
		for (i = 0; i < nSteps; i++) {
			...
			j = (3 * i);
			// consider converting to a float & running through the point
			// convertor if needed */
	 ...
	 * 
	 * @param points
	 *            the <code>PointList</code> to calculate the point on the
	 *            polyline.
	 * @param theDistance
	 ...
	 */
	static public Point pointOn(PointList points, final long theDistance,
			final LineSeg.KeyPoint fromKeyPoint, Point ptResult) {
		List mySegments = getLineSegments(points);
		return pointOn(mySegments, theDistance, fromKeyPoint, ptResult);
	}

	/**
	 * Method getPointsLength. Static utility function to calculate the length
	 * of a series of points if interpreted as line segments.
	 * 
	 * @param points
	...
	/**
	 * Method length. Static utility function to calculate the length of a
	...
	/**
	 * Static utility method to get the nearest segment in the polyline from the
	 * given coordinates.
	 * 
	 * @param mySegments
	 *            the <code>List</code> of <code>LineSeg</code> objects
	 * @param xCoord
	 *            the <code>int</code> x coordinate of the point
	 * @param yCoord
	 *            the <code>int</code> y coordinate of the point
	 * @return the <code>LineSeg</code> by reference which is closest to the
	 *         given coordinates.
	 */
	public static LineSeg getNearestSegment(List mySegments, final int xCoord,
			final int yCoord) {
		long minDistance = BIGDISTANCE;
		long nextDistance = 0;

		LineSeg closeSegment = null;
		LineSeg firstSegment = mySegments.isEmpty() ? null
				: (LineSeg) mySegments.get(0);
		...
		if (theDistance >= thisLength) {
			if (fromKeyPoint == LineSeg.KeyPoint.ORIGIN) {
				((LineSeg) mySegments.get(mySegments.size() - 1)).pointOn(
						theDistance - thisLength, LineSeg.KeyPoint.TERMINUS,
						ptResult);
				return ptResult;
			} else if (fromKeyPoint == LineSeg.KeyPoint.MIDPOINT) {
				((LineSeg) mySegments.get(mySegments.size() - 1)).pointOn(
						theDistance - halfLength, LineSeg.KeyPoint.TERMINUS,
						ptResult);
				return ptResult;
			} else if (fromKeyPoint == LineSeg.KeyPoint.TERMINUS) {
				((LineSeg) mySegments.get(mySegments.size() - 1)).pointOn(
						theDistance, LineSeg.KeyPoint.TERMINUS, ptResult);
				return ptResult;
			} else {
			...
			if (fromKeyPoint == LineSeg.KeyPoint.ORIGIN) {
				((LineSeg) mySegments.get(0)).pointOn(theDistance,
						fromKeyPoint, ptResult);
				return ptResult;
			} else if (fromKeyPoint == LineSeg.KeyPoint.MIDPOINT) {
				return pointOn(mySegments, halfLength + theDistance,
						LineSeg.KeyPoint.ORIGIN, ptResult);
			} else if (fromKeyPoint == LineSeg.KeyPoint.TERMINUS) {
				((LineSeg) mySegments.get(mySegments.size() - 1)).pointOn(
						theDistance, fromKeyPoint, ptResult);
				return ptResult;
			} else {
				IllegalArgumentException iae = new IllegalArgumentException();
				throw iae;
			...
			if (!locateSegment(mySegments, ((double) theDistance)
					/ ((double) thisLength), fromKeyPoint, locateInfo))
				return null;

			locateInfo.theSegment
					.pointOn(
							locateInfo.remainingDist,
							(fromKeyPoint == LineSeg.KeyPoint.MIDPOINT ? LineSeg.KeyPoint.ORIGIN
									: fromKeyPoint), ptResult);
			return ptResult;
		}
	}
	...
	/**
	 * Find the line segment of this polyline that includes the given percentage
	 * of the length of the polyline from the given keypoint. Return a pointer
	 * to the segment and the portion of the length of the return segment to
	...
	private static boolean locateSegment(List mySegments, final double pctDist,
			final LineSeg.KeyPoint fromKeyPoint, LocateInfo locateInfo) {
	...
	protected static double distanceAlong(List mySegments, final Point aPoint) {
		...
		double linePct = segmentDistance(mySegments, theSegment,
	...
	/**
	 * Method segmentDistance. Compute the percentage of the distance along this
	 * polyline that the given segment occurs. If the keypoint specified is
	 * Origin, then the given segment will not be included in the percentage; if
	...
	protected static double segmentDistance(List mySegments,
			LineSeg theSegment, final LineSeg.KeyPoint uptoKeyPoint) {
	...
	/**
	 * Method routeAroundPoint. Calculate a new routed version of this polyline
	 * that will route itself around a given point.
	 * 
	 * @param points
	 *            PointList to modify
	 * @param ptCenter
	 *            the <code>Point</code> around which the routing will occur.
	 * @param nHeight
	 *            the <code>int</code> height to route around the point.
	 * @param nWidth
	 *            the <code>int</code> width to route around the point.
	 * @param nSmoothFactor
	 *            the <code>int</code> smooth factor to route the line with 0 -
	 *            None, 15 - some, 30 - lots
	 * @param nInclineOffset
	 *            the <code>int</code> amount to incline the routed points.
	 * @param bTop
	 *            the <code>boolean</code> route above or below the point on the
	 *            line.
	 * @return <code>PointList</code> that is the newly routed version of
	 *         <code>points</code> of <code>null</code> if operation was not
	 *         successful or if the calculation is not possible.
	 */
	static public PointList routeAroundPoint(PointList points,
			final Point ptCenter, int nHeight, int nWidth, int nSmoothFactor,
			int nInclineOffset, boolean bTop) {
		List mySegments = getLineSegments(points);

		long nPolyLength = length(mySegments);
		long nCenterDistance = Math.round(distanceAlong(mySegments, ptCenter)
				* nPolyLength);

		Point ptMidStart = new Point();
		pointOn(mySegments, nCenterDistance - (nWidth / 2),
				LineSeg.KeyPoint.ORIGIN, ptMidStart);
		Point ptMidEnd = new Point();
		pointOn(mySegments, nCenterDistance + (nWidth / 2),
				LineSeg.KeyPoint.ORIGIN, ptMidEnd);
		...
		LocateInfo locateInfo = new LocateInfo();
		if (!locateSegment(mySegments, (nCenterDistance - ((long) nWidth / 2))
				/ (double) nPolyLength, LineSeg.KeyPoint.ORIGIN, locateInfo))
			return null;
		...
		LineSeg lineStart = new LineSeg(LineSeg.KeyPoint.ORIGIN, // Origin or
																	// Midpoint
				ptStart.x, ptStart.y, lineNew.perpSlope(), nHeight, nDir);

		LineSeg lineEnd = new LineSeg(LineSeg.KeyPoint.ORIGIN, // Origin or
																// Midpoint
		...
		PointList rPolyPoints = new PointList(rRotatedBox.size()
				* MAX_BEZIERLINES + points.size());
		...
		while (lineIter.hasNext()) {
			...
			if (pSegment == pEndSeg) {
				PointList newRoutePoints = new PointList(rRotatedBox.size()
						* MAX_BEZIERLINES);

				LineSeg pCurSeg1 = (LineSeg) boxSegments.get(0);
				LineSeg pCurSeg2 = (LineSeg) boxSegments
						.get(boxSegments.size() - 1);
				getRoutedPoints(points, newRoutePoints, ptMidStart, ptMidEnd,
						ptMidStart, ptMidEnd, pCurSeg1, pCurSeg2, rRotatedBox,
						nSmoothFactor, false, true, 0);
	...
	/**
	 * Method findNearestLineSegIndexOfPoint. Calculate the nearest line segment
	 * index distance wise to the given point.
	 * 
	 * @param points
	 *            PointList to calculate the nearest line segment of.
	 * @param ptCoord
	 *            the <code>Point</code> to test containment of.
	 * @return int Index of line segment that is nearest in the polyline to the
	 *         given point. The index is 1 based where 1 represents the first
	 *         segment.
	 */
	static public int findNearestLineSegIndexOfPoint(PointList points,
			final Point ptCoord) {
		List mySegments = getLineSegments(points);
		ListIterator lineIter = mySegments.listIterator();
		int nNextIndex = 0;
	...
	/**
	 * Method findIntersections. Find all intersection points between this
	 * polyline and another polyline passed into the method.
	 * 
	 * @param points
	 *            PointList to calculate interesections with.
	 * @param poly
	 *            the <code>PointList</code> to calculate intersections with.
	 * @param intersections
	 *            the <code>PointList</code> containing the resulting calculated
	 *            intersection points.
	 * @param distances
	 *            the <code>PointList</code> containing point values
	 *            representing the distance along the polyline the intersections
	 *            occur.
	 * @return boolean true if intersections could be calculated, false
	 *         otherwise.
	 */
	static public boolean findIntersections(PointList points,
			final PointList poly, PointList intersections, PointList distances) {
		...
		while (segIter.hasNext()) {
			...
			while (polyIter.hasNext()) {
				...
				Point ptIntersect = pSegment.intersect(pPolySegment,
						INTERSECT_TOLERANCE);
	...
	/**
	 * Finds a point relative to the pointList passed in based on the parameters
	 * passed in.
	 * 
	 * @param pointList
	 *            the <code>PointList</code>
	 * @param fromLine
	 *            distance off the line
	 * @param fromEnd
	 *            distance from the starting point of the line (i.e. distance
	 *            from source end along the line)
	 * @param isPercentage
	 *            is the fromEnd given as a percentage?
	 * @return Point
	 */
	public static Point calculatePointRelativeToLine(PointList pointList,
			int fromLine, int fromEnd, boolean isPercentage) {
		...
		LocateInfo locateInfo = new LocateInfo();
		if (locateSegment(getLineSegments(pointList), fractionDistance,
				LineSeg.KeyPoint.ORIGIN, locateInfo)) {
			...
			Point location = seg.locatePoint(inSegPercDist, Math.abs(fromLine),
					(fromLine > 0 ? LineSeg.Sign.POSITIVE
	 ...
	 * <Code>PointList</Code> closest to @param p
	 * 
	 * @param points
	 *            - the list of points to select the result from
	 * @param p
	 *            - the point to which the closest point must be found
	 * @return the <Code>Point</Code> from the <Code>PointList</Code> closest to @param
	 *         p
	 */
	 ...
	 */
	static boolean sameOrientation(Point pt1, Point pt2, Point pt3,
			int straightLineTolerance) {
